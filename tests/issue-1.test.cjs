...
test('templates remain conditional; generic-domain state gates use their current value (issue #7)', () => {
  const c=card(); c._hass.states['binary_sensor.motion']={state:'on'};
  assert.equal(predict(c,time('10:00'),[gate('binary_sensor.motion')]).events.length,1);
  assert.equal(predict(c,time('10:00'),[gate('binary_sensor.motion','off')]).events.length,0);
