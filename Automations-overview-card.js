/* Automations Overview Card - V41 - compact and detailed next-planned display modes (issue #9); based on V40 */
...
const AUTOMATION_TIMELINE_I18N = {
  ...
  fr: {
    ...
    certainty_state: "État à confirmer",
  ...
  en: {
    ...
    certainty_state: "State to confirm",
...
class AutomationsOverviewCard extends HTMLElement {
  ...
  _plannedActionSummaries(config) {
    ...
    return actions.filter(action => action && action.enabled !== false).map(action => {
      const service = action.action || action.service;
      if (typeof service === "string") {
        try { return this._formatService({ ...action, service }); }
        catch (_) { return this._humanizeService(service); }
      }
      if (action.delay !== undefined) {
        return this._t("action_delay", { value: this._durationLabel(action.delay) || "…" });
      }
      if (action.choose !== undefined) return this._t("action_choose");
      if (action.if !== undefined || action.then !== undefined) return this._t("action_if");
      if (action.wait_template !== undefined || action.wait_for_trigger !== undefined) return this._t("action_wait");
      if (action.event !== undefined) return this._t("action_event", { value: action.event });
      return this._t("configured_action");
    });
  }
  ...
  _nextPlannedHtml(now, mode = this._displayMode) {
    ...
    const actions = event.actionSummaries?.length
      ? event.actionSummaries.map((action, index) => `
          <li><span class="actionNumber">${index + 1}</span><span>${this._escWithHighlight(action)}</span></li>`).join("")
    ...
    return `
      <div class="nextPlannedShell ${mode === "next_planned_details" ? "detailed" : "compact"}">
        <button class="nextPlanned" data-entity="${this._esc(event.entity_id)}"
          ...
          <span class="nextPlannedTiming">
            <span>${this._esc(exactTime)}</span>
  ...
  _compactSettingsHtml() {
    if (this._displayMode === "full" || !this.config.show_compact_settings) return "";
    return `
      <button id="compactSettingsToggle" class="compactSettingsToggle" title="${this._t("settings_title")}">
        <ha-icon icon="mdi:cog-outline"></ha-icon>
      </button>
      ${this._legendFiltersOpen ? `<div class="compactSettingsPanel">${this._displayModeSelectorHtml()}</div>` : ""}
    `;
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <style>
        ...
        .nextPlanned {
          display: grid;
          grid-template-columns: 24px minmax(150px, 1fr) auto auto 20px;
          gap: 12px;
          align-items: center;
          width: 100%;
        ...
        .conditionSummary li,
        .actionSummary li { display: flex; align-items: flex-start; gap: 6px; margin: 5px 0; }
        ...
        @media (
          ...
          .nextPlannedChevron { grid-column: 3; grid-row: 1 / 4; }
    ...
    if (compactSettingsToggle) {
      compactSettingsToggle.addEventListener("click", event => {
        ...
        this._legendFiltersOpen = !this._legendFiltersOpen;
        this._render();
      });
    }
